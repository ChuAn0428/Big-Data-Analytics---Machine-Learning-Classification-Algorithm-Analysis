...
print('\n2. Dataset description (After cleaning):')
print(df.describe())
...
def bmi_stage(data):
    if (data['weight']/((data['height']/100)**2) < 25):
        return 0
    elif (data['weight']/((data['height']/100)**2) >= 25 and data['weight']/((data['height']/100)**2) <= 30):
        return 1
    elif (data['weight']/((data['height']/100)**2) >= 30 and data['weight']/((data['height']/100)**2) <= 35):
        return 2
    elif (data['weight']/((data['height']/100)**2) >= 35 and data['weight']/((data['height']/100)**2) <= 40):
        return 3
    elif (data['weight']/((data['height']/100)**2) >= 40):
        return 4   
  
def hypertension_stage(data):
    if (data['ap_hi'] <= 130) and (data['ap_lo'] <= 80):
        return 0
    elif (data['ap_hi'] > 130 and data['ap_hi'] <= 139) and (data['ap_lo'] <= 80):
        return 1
    elif (data['ap_hi'] >= 140 and data['ap_hi'] <= 149) | (data['ap_lo'] > 80 and data['ap_lo'] <= 89):
        return 2
    elif (data['ap_hi'] >= 150 and data['ap_hi'] <= 159) | (data['ap_lo'] >= 90 and data['ap_lo'] <= 99):
        return 3
    elif (data['ap_hi'] >= 160) | (data['ap_lo'] >= 100):
        return 4
...
def age_stage(data):
    if (data['age'] <= 35):
        ...
        return 2
    elif (data['age'] > 55 and data['age'] <= 65):
        return 3


#df['BMI'] = (df['weight']/((df['height']/100)**2)).apply(np.log2)
df['age'] = df.apply(age_stage, axis=1)
df['obesity'] = df.apply(bmi_stage, axis=1)
df['hypertension'] = df.apply(hypertension_stage, axis=1)

df['class'] = df['cardio']
    
df_engir = df.drop(['cardio','id','height','weight','ap_hi', 'ap_lo'], axis=1)
#df_engir = df.drop(['cardio','id','height','weight'], axis=1)

print('\n3. Dataset description (After feature engirneering):')
print(df_engir.describe())
...
data = df_engir.values.copy()
#x = data[:,1:-1].copy()

pca = PCA(n_components=7)
x = pca.fit_transform(data[:,0:-1].copy())  
...
rfc = RandomForestClassifier(n_estimators=100, max_depth=None, min_samples_split=2, random_state=0).fit(x_train, y_train)
